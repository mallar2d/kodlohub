...
const AnalyticsCharts = dynamic(
  () => import("@/components/admin/AnalyticsCharts"),
  {
    ssr: false,
    loading: () => (
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-12 animate-pulse">
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="card-dark p-6 h-[262px] bg-canvas-night-soft/50" />
        ))}
      </div>
    ),
  }
  ...
  return (
    <div className="min-h-screen pt-24 pb-16 px-4 sm:px-6">
      <div className="max-w-[1200px] mx-auto">
        ...
        <AnalyticsCharts stats={stats} actionLabels={actionLabels} />
      </div>
    </div>
  );
}
