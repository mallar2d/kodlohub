...
export default function HomePage() {
  ...
  const [stats, setStats] = useState<Stats>({ mediaCount: 0, postsCount: 0, profilesCount: 0 });
  ...
  useEffect(() => {
    async function fetchData() {
      const [mediaRes, postsRes, profilesRes] = await Promise.all([
        supabase.from("media").select("id, file_url, file_type, caption, created_at").order("created_at", { ascending: false }).limit(6),
        supabase.from("posts").select("id, title, content, tags, created_at").order("created_at", { ascending: false }).limit(3),
      ...
      <section className="px-6 pb-16">
        <div className="max-w-[1200px] mx-auto">
          <div className="flex items-center justify-between mb-8">
            ...
            <Link href="/gallery" className="micro-cap text-on-primary-mute hover:text-on-primary transition-opacity">
              ДИВИТИСЬ ВСЕ →
            </Link>
          </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {recentMedia.map((item) => (
                ...
                >
                  ...
                  {item.caption && (
                    <div className="p-3">
                      <p className="caption text-on-primary-mute line-clamp-1">{item.caption}</p>
                    </div>
                  )}
                </Link>
              ))}
      ...
      <section className="px-6 pb-16">
        <div className="max-w-[1200px] mx-auto grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            ...
            { href: "/lore", title: "ЛОРА", desc: "Архів артефактів та мемів" },
