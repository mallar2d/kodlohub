...
function BratTdLinkInner() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(({ data }) => {
      if (!data.user) {
        const next = `/brat-td/link${initial ? `?code=${initial}` : ""}`;
  ...
  const confirm = async () => {
    ...
    try {
      ...
      setStatus("ok");
      setMessage("Brat TD підключено. Повернись у гру — прогрес синхронізується з хмарою.");
    } catch {
      setStatus("error");
  ...
  return (
    <main className="min-h-screen pt-24 pb-16 px-4 sm:px-6">
      <div className="max-w-lg mx-auto">
        <p className="micro-cap text-ink-mute mb-2">BRAT TD</p>
        <h1 className="heading-section mb-4">ПІДКЛЮЧИТИ ГРУ</h1>
        <p className="text-on-primary-mute text-sm mb-8">
          Введи код з Brat TD (desktop, веб або Hammer Launcher), щоб прив&apos;язати
          акаунт KodloHUB. Прогрес із локальної гри синхронізується в хмару.
        </p>
        ...
        <div className="mt-8 flex flex-wrap gap-4 text-sm">
          ...
          <Link href="/tools/brat-td" className="text-on-primary-mute hover:text-on-primary">
            Грати в браузері
          </Link>
          <Link href="/brat-td/" className="text-on-primary-mute hover:text-on-primary">
            Повний екран
          </Link>
        </div>
      </div>
...
export default function BratTdLinkPage() {
  return (
    <Suspense
      fallback={
