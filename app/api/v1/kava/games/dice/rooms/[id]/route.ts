...
export async function DELETE(
  ...
  try {
    ...
    if (room.status !== "waiting") {
      return NextResponse.json({ success: false, message: "Неможливо скасувати активну або завершену гру" }, { status: 400 });
    }

    await admin.from("dice_rooms").update({ status: "cancelled" }).eq("id", roomId);

    return NextResponse.json({ success: true, message: "Кімнату скасовано" });
  } catch (error: any) {
